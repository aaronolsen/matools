parse_unify_spec <- function(ulist, xr_arr, ct_mat, regexp = FALSE, print.progress = FALSE){
	...
	for(elem_name in names(ulist)){
		...
		# If list but no names treat as unnamed list element
		if(is.list(ulist[[elem_name]])){
		
			# 
			lengths <- unlist(lapply(ulist[[elem_name]], 'length'))
			
			if(is.null(names(ulist[[elem_name]])) && !any(lengths != 1)){

				# Unlist and convert to single list with group of align landmarks
				ulist[[elem_name]] <- list('align'=unlist(ulist[[elem_name]]))

			}else{

				# No names and more than one element per?
				if(is.null(names(ulist[[elem_name]]))) stop("Sub-groups within an element in unify specification must be named (e.g. 'align', 'point', 'orient').")

				if(length(ulist[[elem_name]]) == 1){
					if(is.null(names(ulist[[elem_name]]))) ulist[[elem_name]][['align']] <- ulist[[elem_name]][[1]]
				}
			}

		}else{

			# Convert to list with single group of align landmarks
			ulist[[elem_name]] <- list('align'=ulist[[elem_name]])
		}
		...
		if(regexp){
			...
			for(l_type in names(ulist[[elem_name]])){
				...
				if(is.list(ulist[[elem_name]][[l_type]])){
					...
					for(sub_elem in names(ulist[[elem_name]][[l_type]])){
						...
						# Add markers to parent-child matrix
						if(l_type %in% c('point', 'plane')){
	...
	for(elem_name in names(ulist)){
		...
		if(!is.null(ulist[[elem_name]][['plane']])){
			...
			sub_elem <- names(ulist[[elem_name]][['plane']])[names(ulist[[elem_name]][['plane']]) != ""]
	...
	parent_children <- parent_children[order(parent_children[,1]),]
